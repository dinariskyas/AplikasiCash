package com.dinariskyalinsaputri.aplikasicash.helper;
...
//    private static final int DATABASE_VERSION = 2; //upgraded
    ...
    // Database Name
    private static final String DATABASE_NAME = "AplikasiCash.db";
...
//            values.put(COLUMN_USER_FULL_NAME, user.getFullname());
    ...
    public void addCashFlow(DetailCash dcf) {
        ContentValues values = new ContentValues();
    ...
    public List<DetailCash> getNominalPemasukanByBulan(String bulan) {
...
//                COLUMN_CASH_FLOW_TIPE
        ...
        List<DetailCash> detailCashList = new ArrayList<DetailCash>();
        SQLiteDatabase db = this.getReadableDatabase();
        ...
        if (cursor.getCount() > 0) {
            if (cursor.moveToFirst()) {
                do {
                    DetailCash dcfPemasukan = new DetailCash();
...
//                    dcfPemasukan.setTipe(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_CASH_FLOW_TIPE)));
                    ...
                    detailCashList.add(dcfPemasukan);
...
//                return detailCashFlowList;
        ...
        return detailCashList;
    }
    public List<DetailCash> getNominalPengeluaranByBulan(String bulan) {
...
//                COLUMN_CASH_FLOW_TIPE
        ...
        if (cursor.getCount() > 0) {
            if (cursor.moveToFirst()) {
                do {
                    DetailCash dcfPengeluaran = new DetailCash();
...
//                    dcfPengeluaran.setTipe(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_CASH_FLOW_TIPE)));
                    ...
                    // Adding detail cash flow record to list
                    detailCashList.add(dcfPengeluaran);
                    //                total += Integer.parseInt(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_CASH_FLOW_NOMINAL)));
                } while (cursor.moveToNext());
                cursor.close();
                db.close();
//                return detailCashFlowList;
    ...
    public List<DetailCash> getAllCashFlow(){
